...
export const getDashboardData = async (
  ...
  const [customersRes, purchasesRes] = await Promise.all([
    customersApi.get<Customer[]>(
      userId ? `/customers?ownerId=${userId}` : "/customers",
    ),
  ...
  const filteredPurchases = userId
    ? purchases.filter((p) =>
        customers.some((c) => c.id === p.customerId.toString()),
      )
    : purchases;
  ...
  const purchasesChartData = Object.entries(salesByDay).map(
    ([date, totalAmount]) => ({ date, totalAmount }),
  );
  purchasesChartData.sort(
